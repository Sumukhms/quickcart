      ...
      >
        ...
        <div>
          ...
          <p
            className="text-sm font-bold"
            style={{
              color: rs === "refunded" ? "#22c55e" : "var(--text-secondary)",
            }}
          >
...
export default function UserOrders() {
  ...
  const handleReorder = useCallback(
    async (order) => {
      ...
      if (cartStore && cartStore._id !== (store._id || store)) {
        if (!window.confirm(`Clear cart and reorder from "${store.name}"?`))
          return;
        clearCart();
      }
  ...
  const TABS = [
    ...
    { id: "cancelled", label: "Cancelled", count: counts.cancelled },
    {
      id: "refunds",
      label: "Refunds",
      count: orders.filter((o) => o.refundStatus && o.refundStatus !== "none")
        .length,
    },
  ];
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {activeCount > 0 && (
          ...
          >
            ...
            <button
              onClick={() => setSearchQuery("") || setFilter("active")}
            ...
            >
              ...
              {count > 0 && (
                <span
                  ...
                  style={{
                    background:
                      filter === id ? "rgba(255,255,255,0.2)" : "var(--border)",
                    color: filter === id ? "white" : "var(--text-muted)",
                  }}
                >
                  {count}
              ...
              >
                ...
                <div className="flex-1">
                  <p
                    ...
                    style={{ color: "var(--text-primary)" }}
                  >
                    {refundOrders.length} refund
                    {refundOrders.length !== 1 ? "s" : ""}
                  </p>
                  <p
                  ...
                  >
                    ...
                    {refundOrders
                      .reduce(
                        (s, o) => s + (o.refundAmount || o.totalPrice || 0),
                        0,
                      )
                      .toFixed(0)}
                    &nbsp;·&nbsp;
                    {
                      refundOrders.filter((o) => o.refundStatus === "refunded")
                        .length
                    }{" "}
                    completed
                  </p>
          ...
          <EmptyState
            ...
            action={
              searchQuery ? (
                ...
                <Link to="/user/home" className="btn btn-brand text-sm">
                  Browse Stores
                </Link>
              )
            }
                  ...
                  <div className="p-5">
                      ...
                      <div className="flex flex-col items-end gap-1.5">
                        ...
                        {hasRefund && (
                          <span
                            ...
                            style={{
                              background:
                                REFUND_STATUS_CONFIG[order.refundStatus]?.bg ||
                                "var(--elevated)",
                              color:
                                REFUND_STATUS_CONFIG[order.refundStatus]
                                  ?.color || "var(--text-muted)",
                            }}
                          >
                            💳{" "}
                            {REFUND_STATUS_CONFIG[order.refundStatus]?.label ||
                              "Refund"}
                          </span>
                        )}
                      </div>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {/* Invoice Download (delivered only) */}
                        {order.status === "delivered" && (
                          <button
                            onClick={() => downloadInvoice(order._id)}
                            disabled={invoiceLoading}
                            className="flex items-center gap-1 text-xs font-semibold px-2 py-1.5 rounded-lg transition-all hover:scale-105"
                            style={{
                              background: "rgba(59,130,246,0.1)",
                              color: "#3b82f6",
                            }}
