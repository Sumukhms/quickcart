...
export default function UserOrders() {
  ...
  const handleReorder = useCallback(async (order) => {
    ...
    for (const item of order.items) {
      const product = { _id: item.productId, name: item.name, price: item.price, image: item.image || "", available: true, unit: item.unit || "" };
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {order.status === "delivered" && (
                          <button onClick={() => handleReorder(order)} disabled={reordering === order._id}
                            ...
                            {reordering === order._id ? <Loader2 size={12} className="animate-spin" /> : <><RotateCcw size={12} /> Reorder</>}
                          </button>
                        )}
                        ...
                        {canCancel && (
                          <button onClick={() => handleCancel(order._id)} disabled={cancelling === order._id}
                            ...
                            {cancelling === order._id ? <Loader2 size={12} className="animate-spin" /> : <><Ban size={12} /> Cancel</>}
                          </button>
                        )}
                        {/* Track / Details */}
                        <Link to={`/user/orders/${order._id}`}
                          className="flex items-center gap-1 text-sm font-semibold" style={{ color: "var(--brand)" }}>
