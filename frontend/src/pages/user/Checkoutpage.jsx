...
export default function CheckoutPage() {
  ...
  const couponDiscount = appliedCoupon
    ? freeDelivery ? DELIVERY_FEE : appliedCoupon.discountAmount
    : 0;
  ...
  const placeOrder = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (err.message === "Payment cancelled by user") {
          ...
          navigate(`/payment/failure?type=failed&message=${encodeURIComponent(err.message)}`);
        }
        return;
      }
    ...
    selectedAddrObj, selectedAddr, paymentMethod, notes, appliedCoupon,
    clearCart, addToast, cartItems, cartStore, grandTotal, user, initiatePayment, navigate,
  ]);
              ...
              {step === 2 && (
                <div className="px-5 pb-5 pt-2 space-y-3">
                  {PAYMENT_METHODS.map(({ id, label, sub, icon: Icon, color }) => (
                    <button
                      key={id}
                      onClick={() => { setPaymentMethod(id); setStockError(null); }}
