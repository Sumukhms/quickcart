 ...
 *
 * Changes:
 *   1. Replaced inline address section with <AddressSelector>
 *   2. Replaced inline order bill with <OrderSummary> on sidebar
 *   3. Reads user.addresses[] for saved addresses
 *   4. Persists new addresses to backend via authAPI.addAddress
 *
 * Everything else (steps, payment, coupon, place order) is unchanged.
 */
...
export default function CheckoutPage() {
  ...
  const placeOrder = useCallback(async () => {
    ...
    // ── Shared order payload (same shape as original) ──────────
    const orderData = {
    ...
    try {
      // ── COD: use the original direct-order path ────────────
      ...
      // Step B: open Razorpay checkout popup
      let paymentResponse;
      try {
        ...
        // User cancelled popup or payment failed
      ...
      // Step C: verify on backend — backend creates the DB order
      let order;
      try {
    ...
    >
      <div className="max-w-5xl mx-auto px-4 py-6 pb-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,380px] gap-5">
            ...
            >
              ...
              {step === 1 && (
                <div className="px-5 pb-5 pt-3 space-y-4">
                  {/* ── NEW: AddressSelector replaces inline textarea ── */}
                  <AddressSelector
                    addresses={addresses}
                  ...
                  <div className="space-y-2 mb-4 max-h-40 overflow-y-auto">
                    {cartItems.map((item) => (
                      <div key={item._id} className="flex items-center gap-3">
                        <div
                          className="w-7 h-7 rounded-lg flex-shrink-0 overflow-hidden img-fallback text-xs"
                          style={{ background: "var(--elevated)" }}
                        >
                          {item.image ? (
                            <img
          ...
          {/* ── Order Summary sidebar (reusable component) ── */}
          <div className="lg:sticky lg:top-24 h-fit space-y-3">
            ...
            >
              ...
              {/* ── NEW: reusable OrderSummary component ── */}
              <OrderSummary
