  ...
  return (
    ...
    >
      <div className="text-center max-w-sm w-full">
        ...
        <div
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl mb-6 font-mono text-sm"
        ...
        >
          ...
          >
            <div
              ...
              style={{ width: "25%", background: "var(--brand)" }}
            />
          </div>
          <div
      ...
      const valid = (Array.isArray(data) ? data : []).filter(
        (c) =>
          ...
          (c.applicableCategories.length === 0 ||
            c.applicableCategories.includes(storeCategory)),
      );
      setCoupons(valid);
      setFetched(true);
    } catch {
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-0.5">
                    <span
                      className="font-black text-xs font-mono tracking-wider px-2 py-0.5 rounded-md"
                      style={{
...
export default function CheckoutPage() {
  ...
  // ✅ FIX: store full address object, derive string from it
  const [selectedAddrObj, setSelectedAddrObj] = useState(null);
  // ✅ FIX: derive formatted address string from the selected object
  ...
  const couponDiscount =
    appliedCoupon && !freeDelivery ? appliedCoupon.discountAmount || 0 : 0;
  const effectiveDel = freeDelivery ? 0 : DELIVERY_FEE;
  // ✅ FIX: grandTotal = subtotal + delivery - discount (no double-counting)
  const grandTotal = Math.max(0, total + effectiveDel - couponDiscount);

  const applyCoupon = useCallback(
    async (codeArg) => {
      const code = (typeof codeArg === "string" ? codeArg : couponInput)
        .trim()
        .toUpperCase();
      if (!code) return;
      ...
      try {
        ...
        setCouponInput("");
        addToast(
          `Coupon applied! ${data.coupon.description || "Discount applied"}`,
          "success",
        );
      } catch (err) {
  ...
  const placeOrder = useCallback(async () => {
    ...
    // ✅ FIX: build orderData with correct address fields
    const orderData = {
      storeId: cartStore?._id,
      items: serialisedItems,
      totalPrice: grandTotal,
      deliveryAddress: selectedAddr,
      // ✅ Only pass addressId if it's a valid structured address (has MongoDB _id)
      ...(selectedAddrObj?._id && String(selectedAddrObj._id).length === 24
        ? { addressId: selectedAddrObj._id }
        : {}),
      paymentMethod,
      notes: notes?.trim() || undefined,
      couponCode: appliedCoupon?.code || undefined,
      // ✅ GPS coords from structured address
    ...
    try {
      ...
      try {
        ...
        if (resData?.stockError || err.response?.status === 400) {
          ...
          addToast(
            resData?.message || "Stock issue — please update your cart",
            "error",
          );
          return;
        }
        if (resData?.configError) {
          addToast(
            "Payment system not configured. Please use Cash on Delivery.",
            "error",
          );
        ...
        addToast(
          resData?.message || "Could not initiate payment. Please try again.",
          "error",
        );
        return;
      }
      ...
      try {
        ...
        if (err.message === "Payment cancelled by user") {
          ...
          navigate(`/payment/failure?type=cancelled`);
        } else {
          navigate(
            `/payment/failure?type=failed&message=${encodeURIComponent(err.message)}`,
          );
        }
        return;
      }
      ...
      try {
        ...
        if (resData?.stockError || resData?.needsRefund) {
          navigate(
            `/payment/failure?type=stock&orderId=${paymentResponse.razorpay_order_id}&paymentId=${paymentResponse.razorpay_payment_id}&message=${encodeURIComponent(resData.message)}`,
          );
          return;
        }
        addToast(
          resData?.message || "Payment verification failed. Contact support.",
          "error",
        );
        return;
      }
  ...
  if (placedOrder) {
    return (
      <OrderSuccess
        order={placedOrder}
        onContinue={() =>
          navigate(POST_ORDER_ROUTE[user?.role] || "/user/orders")
        }
      />
    );
  }
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <Link to="/user/home" className="btn btn-brand">
            Browse Stores
          </Link>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,380px] gap-5">
          <div className="space-y-4">
            ...
            <div
              ...
              style={{
                ...
                border: `1.5px solid ${step === 1 ? "var(--brand)" : "var(--border)"}`,
              }}
            >
              <button
                onClick={() => setStep(1)}
                ...
                style={{
                  borderBottom: step === 1 ? "1px solid var(--border)" : "none",
                }}
              >
                <div
                  ...
                  style={{
                    background:
                      step === 1 ? "rgba(255,107,53,0.1)" : "var(--elevated)",
                  }}
                >
                  📍
                </div>
              ...
              {step === 1 && (
                <div className="px-5 pb-5 pt-3 space-y-4">
                  {/* ✅ FIX: pass selectedAddrObj._id as selected, receive full object in onSelect */}
                  <AddressManager
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Ring bell twice, 3rd floor"
                      value={notes}
            ...
            <div
              ...
              style={{
                backgroundColor: "var(--card)",
                border: `1.5px solid ${step === 2 ? "var(--brand)" : "var(--border)"}`,
            ...
            >
              <button
                ...
                className="w-full flex items-center gap-3 px-5 py-4 text-left"
                style={{
                  borderBottom: step === 2 ? "1px solid var(--border)" : "none",
                }}
              >
                <div
                  className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{
                    background:
                      step === 2 ? "rgba(255,107,53,0.1)" : "var(--elevated)",
                  }}
                >
                  <CreditCard
                    size={15}
                    style={{
                      color: step === 2 ? "var(--brand)" : "var(--text-muted)",
                    }}
                  />
                </div>
                <div className="flex-1">
                  ...
                  <div className="pt-2">
                    ...
                    {appliedCoupon ? (
                      ...
                      <>
                        <AvailableCoupons
                          total={total}
                          storeCategory={cartStore?.category}
                          onApply={applyCoupon}
                        />
            ...
            <div
              ...
              style={{
                backgroundColor: "var(--card)",
                border: `1.5px solid ${step === 3 ? "var(--brand)" : "var(--border)"}`,
            ...
            >
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      step === 3 ? "rgba(255,107,53,0.1)" : "var(--elevated)",
                  }}
                >
                  <Package
                    size={15}
                    style={{
                      color: step === 3 ? "var(--brand)" : "var(--text-muted)",
                    }}
                  />
                </div>
                <p
              ...
              {step === 3 && (
                <div className="px-5 pb-5 pt-2">
                  <div
                    className="rounded-xl p-3 mb-4 space-y-1 text-xs"
                    style={{
                      background: "var(--elevated)",
                      color: "var(--text-muted)",
                    }}
                  ...
                  <div className="space-y-2 mb-4 max-h-40 overflow-y-auto">
                    {cartItems.map((item) => (
                      <div key={item._id} className="flex items-center gap-3">
                        <div
                          className="w-7 h-7 rounded-lg flex-shrink-0 overflow-hidden flex items-center justify-center text-xs"
                          style={{ background: "var(--elevated)" }}
                        >
                          {item.image ? (
                            <img
                  ...
                  {stockError && (
                    <div
                      className="rounded-xl p-3 mb-3 flex items-start gap-2 text-xs"
                      style={{
                        background: "rgba(239,68,68,0.08)",
                        color: "#ef4444",
                  ...
                  >
                    {placing ? (
                      ...
                      <>
                        <Lock size={16} /> Pay ₹{grandTotal.toFixed(0)} via{" "}
          ...
          <div className="lg:sticky lg:top-24 h-fit space-y-3">
            ...
            >
              ...
              {/* ✅ FIX: pass items with quantity (not qty) for OrderSummary */}
              <OrderSummary
