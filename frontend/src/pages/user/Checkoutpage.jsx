...
import {
  ...
  Tag, CheckCircle, Zap, Lock, ChevronRight, X,
  Clock, Package, ArrowRight, Loader2, AlertCircle,
} from "lucide-react";
import { useCart }           from "../../context/CartContext";
import { useAuth }           from "../../context/AuthContext";
import { orderAPI, couponAPI, authAPI } from "../../api/api";
...
export default function CheckoutPage() {
  ...
  }, [selectedAddr, paymentMethod, notes, appliedCoupon, clearCart, addToast, cartItems, cartStore, grandTotal]);

  if (placedOrder)
    return <OrderSuccess order={placedOrder} onContinue={() => navigate("/user/home")} />;
                  ...
                  <div className="space-y-2 mb-4 max-h-40 overflow-y-auto">
                    {cartItems.map((item) => (
                      <div key={item._id} className="flex items-center gap-3">
                        <div className="w-7 h-7 rounded-lg flex-shrink-0 overflow-hidden img-fallback text-xs"
                        ...
                        <span className="text-xs px-1.5 py-0.5 rounded" style={{ background: "var(--elevated)", color: "var(--text-muted)" }}>×{item.qty}</span>
                        <p className="text-sm font-bold w-14 text-right" style={{ color: "var(--text-primary)" }}>₹{item.price * item.qty}</p>
                      </div>
                    ))}
                  </div>
