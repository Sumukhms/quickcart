...
const PAYMENT_METHODS = [
  {
    ...
    icon: Banknote, color: "#22c55e", available: true,
  },
  {
    id: "upi", label: "UPI Payment", sub: "GPay, PhonePe, Paytm & more",
    icon: Smartphone, color: "#3b82f6", available: true,
  },
  {
    id: "card", label: "Credit / Debit Card", sub: "All major cards accepted",
    icon: CreditCard, color: "#8b5cf6", available: true,
  },
];

const COUPONS = {
  QUICKFIRST: { discount: 0.15, label: "15% off your first order", max: 100 },
  SAVE50: { discount: 0, flat: 50, label: "₹50 flat off (min ₹199)", minOrder: 199 },
  FREEDEL: { discount: 0, freeDelivery: true, label: "Free delivery on this order" },
};
    ...
    <div className="min-h-screen flex items-center justify-center px-4 page-enter" style={{ backgroundColor: "var(--bg)" }}>
      <div className="text-center max-w-sm w-full">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl mb-6 font-mono text-sm"
          ...
          Order #{order?._id?.slice(-8).toUpperCase() || "DEMO1234"}
        </div>
        ...
        <div className="rounded-2xl p-4 mb-6 text-left"
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--elevated)" }}>
            <div className="h-full rounded-full" style={{ width: "25%", background: "var(--brand)", animation: "progressBar 2s ease-in-out" }} />
          </div>
...
export default function CheckoutPage() {
  ...
  const couponDiscount = couponData
    ? (couponData.freeDelivery ? DELIVERY_FEE : (couponData.flat || Math.min(Math.round(total * couponData.discount), couponData.max || Infinity)))
    : 0;
  const effectiveDelivery = couponData?.freeDelivery ? 0 : DELIVERY_FEE;
  const grandTotal = total + effectiveDelivery - (couponData?.flat || (couponData && !couponData.freeDelivery ? Math.min(Math.round(total * couponData.discount), couponData.max || Infinity) : 0));
  ...
  const applyCoupon = () => {
    ...
    if (data.minOrder && total < data.minOrder) { setCouponError(`Minimum order ₹${data.minOrder} required`); return; }
    setAppliedCoupon(code);
    setCouponError("");
    setCouponInput("");
    addToast(`Coupon applied! ${data.label}`, "success");
  };

  const placeOrder = async () => {
    if (!address.trim()) { addToast("Please enter delivery address", "error"); return; }
    setPlacing(true);
    try {
      ...
      clearCart();
      setPlacedOrder({ _id: "demo_" + Date.now() });
    } finally { setPlacing(false); }
  };

  if (placedOrder) {
    return <OrderSuccess order={placedOrder} onContinue={() => navigate("/")} />;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 page-enter" style={{ backgroundColor: "var(--bg)" }}>
        <div className="text-center">
          ...
          <p className="mb-5" style={{ color: "var(--text-muted)" }}>Add some items before checking out</p>
          <Link to="/" className="btn btn-brand">Browse Stores</Link>
        </div>
      </div>
    );
  }
            ...
            <div className="rounded-3xl overflow-hidden"
              ...
              {step === 1 && (
                <div className="px-5 pb-5 pt-2 space-y-3">
                  ...
                  {user?.address && (
                    <button
                      onClick={() => { setAddress(user.address); setCustomAddress(false); }}
                      className="w-full flex items-start gap-3 p-3.5 rounded-2xl text-left transition-all hover:scale-[1.01]"
                      style={{
                        background: !customAddress ? "rgba(255,107,53,0.06)" : "var(--elevated)",
                        border: `1.5px solid ${!customAddress ? "var(--brand)" : "var(--border)"}`,
                      }}>
                      <MapPin size={15} style={{ color: "var(--brand)", flexShrink: 0, marginTop: 2 }} />
                      <div>
                        <p className="text-xs font-bold mb-0.5" style={{ color: "var(--text-muted)" }}>SAVED ADDRESS</p>
                        <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>{user.address}</p>
                      </div>
                      {!customAddress && (
                        <div className="ml-auto flex-shrink-0">
                          <div className="w-4 h-4 rounded-full" style={{ background: "var(--brand)" }}>
                            <Check size={10} style={{ color: "white", margin: "3px" }} />
                          </div>
                        </div>
                      )}
                    </button>
                  ...
                  <button onClick={() => setCustomAddress(true)}
                    className="flex items-center gap-2 text-sm font-semibold transition-all"
                    style={{ color: customAddress ? "var(--brand)" : "var(--text-muted)" }}>
                  ...
                  <div className="pt-2">
                    ...
                    {appliedCoupon ? (
                      <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-bold" style={{ color: "#22c55e" }}>{appliedCoupon}</p>
                          <p className="text-xs" style={{ color: "var(--text-muted)" }}>{COUPONS[appliedCoupon].label}</p>
                        </div>
                        <button onClick={() => { setAppliedCoupon(null); addToast("Coupon removed", "info"); }}
                          className="p-1 rounded-lg transition-all hover:scale-110" style={{ color: "var(--text-muted)" }}>
                          <X size={14} />
                        </button>
                      </div>
                    ) : (
                      <div className="flex gap-2">
                        <input
                          ...
                          placeholder="Try QUICKFIRST, SAVE50, FREEDEL"
                          value={couponInput}
                          onChange={e => { setCouponInput(e.target.value.toUpperCase()); setCouponError(""); }}
                          onKeyDown={e => e.key === "Enter" && applyCoupon()}
                        />
                        <button onClick={applyCoupon} className="btn btn-brand text-sm px-4 py-2.5 flex-shrink-0">
                          Apply
                        </button>
                      </div>
                    )}
            ...
            <div className="rounded-3xl overflow-hidden"
              ...
              <div className="px-5 py-4 space-y-2" style={{ borderTop: "1px solid var(--border)" }}>
                {[
                  ...
                  { label: "Delivery fee", value: effectiveDelivery === 0 ? "FREE" : `₹${effectiveDelivery}`, green: effectiveDelivery === 0 },
                  ...(couponData && !couponData.freeDelivery
                    ? [{ label: `Coupon (${appliedCoupon})`, value: `-₹${couponDiscount}`, green: true }]
                    : []),
                ].map(({ label, value, green }) => (
                  <div key={label} className="flex justify-between text-sm">
                    <span style={{ color: "var(--text-secondary)" }}>{label}</span>
                    <span className="font-semibold" style={{ color: green ? "#22c55e" : "var(--text-primary)" }}>{value}</span>
                  </div>
                ))}
              ...
              {(couponData || true) && (
                <div className="mx-4 mb-4 px-4 py-2.5 rounded-xl flex items-center gap-2 text-xs font-semibold"
                  ...
                  🎉 {couponData ? `You saved ₹${couponDiscount}!` : "Add a coupon code to save more"}
                </div>
              )}
            </div>
          </div>
...
// Missing import
function Check({ size, style }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" style={style}><polyline points="20 6 9 17 4 12"></polyline></svg>;
}