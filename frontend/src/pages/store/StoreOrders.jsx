...
import {
  ChevronLeft, RefreshCw, Search, Filter, Check, X,
  Phone, Clock, ChevronDown, ChevronUp, Package
} from "lucide-react";
...
const DEMO_ORDERS = [
  { _id: "o1", status: "pending",   totalPrice: 245, deliveryFee: 20, createdAt: new Date().toISOString(),                  userId: { name: "Raj Kumar",   phone: "+91 98765 43210" }, items: [{ name: "Amul Milk 500ml", quantity: 2, price: 28 }, { name: "Bread", quantity: 1, price: 45 }], deliveryAddress: "12, 3rd Main, Koramangala", paymentMethod: "cod" },
  { _id: "o2", status: "preparing", totalPrice: 180, deliveryFee: 20, createdAt: new Date(Date.now()-1800000).toISOString(), userId: { name: "Priya Singh", phone: "+91 87654 32109" }, items: [{ name: "Basmati Rice 1kg", quantity: 1, price: 120 }, { name: "Tata Salt", quantity: 1, price: 22 }], deliveryAddress: "5B, 1st Cross, HSR Layout", paymentMethod: "cod" },
  { _id: "o3", status: "delivered", totalPrice: 320, deliveryFee: 20, createdAt: new Date(Date.now()-86400000).toISOString(), userId: { name: "Arjun Mehta", phone: "+91 76543 21098" }, items: [{ name: "Mixed Veggies", quantity: 2, price: 80 }, { name: "Eggs 12pcs", quantity: 1, price: 90 }], deliveryAddress: "44, 8th Main, Indiranagar", paymentMethod: "online" },
  { _id: "o4", status: "confirmed", totalPrice: 95,  deliveryFee: 0,  createdAt: new Date(Date.now()-3600000).toISOString(), userId: { name: "Sneha Patel", phone: "+91 65432 10987" }, items: [{ name: "Parle-G Biscuits", quantity: 3, price: 10 }, { name: "Sprite 750ml", quantity: 1, price: 45 }], deliveryAddress: "77, 2nd Phase, JP Nagar", paymentMethod: "cod" },
  { _id: "o5", status: "cancelled", totalPrice: 150, deliveryFee: 20, createdAt: new Date(Date.now()-7200000).toISOString(), userId: { name: "Vikram Das",  phone: "+91 54321 09876" }, items: [{ name: "Fortune Oil 1L", quantity: 1, price: 145 }], deliveryAddress: "9, 6th Sector, HSR Layout", paymentMethod: "cod" },
];
      ...
      {expanded && (
        <div className="px-5 pb-4 pt-0" style={{ borderTop: "1px solid var(--border)" }}>
          ...
          <div className="py-3 space-y-1.5">
            ...
            <div className="flex justify-between text-xs pt-1" style={{ borderTop: "1px solid var(--border)", color: "var(--text-muted)" }}>
          ...
          {nextStatus && order.status !== "cancelled" && (
            <div className="flex gap-2">
              <button onClick={() => onStatusUpdate(order._id, nextStatus)}
                ...
                <Check size={12} /> → {STATUS_CONFIG[nextStatus]?.label}
              </button>
...
export default function StoreOrders() {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    addToast(`Order → ${STATUS_CONFIG[newStatus]?.label}`, "success");
  };
    ...
    <div className="min-h-screen page-enter" style={{ backgroundColor: "var(--bg)" }}>
      <div className="max-w-3xl mx-auto px-4 py-6 pb-20">
        ...
        <div className="relative mb-4">
          ...
          <input className="input-theme pl-10 text-sm py-2.5" placeholder="Search by customer or item…"
            ...
            <button key={id} onClick={() => setStatusFilter(id)}
              ...
              {count > 0 && (
                <span className="px-1.5 py-0.5 rounded-md text-[10px] font-bold"
                  style={{ background: statusFilter === id ? "rgba(255,255,255,0.2)" : "var(--border)", color: statusFilter === id ? "white" : "var(--text-muted)" }}>
                  {count}
                </span>
              )}
            </button>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map(order => (
              <OrderRow key={order._id} order={order} onStatusUpdate={updateOrderStatus} />
            ))}
          </div>
        )}
      </div>
