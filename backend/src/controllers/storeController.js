...
export const createStore = async (req, res) => {
  try {
    const { name, phone, address, category, image, deliveryTime, minOrder } = req.body;
    if (!name || !phone || !address || !category) return res.status(400).json({ message: "Missing fields" });
    const store = await Store.create({ name, phone, address, category, image, deliveryTime, minOrder, ownerId: req.user.userId });
